...
import static com.didichuxing.doraemonkit.constant.BundleKey.TYPE_BLOCK;
...
public class PerformanceListAdapter extends AbsRecyclerAdapter<AbsViewBinder<PerformanceBean>, PerformanceBean> {
    private int mType = TYPE_FRAME;

    public PerformanceListAdapter(Context context, int type) {
        super(context);
        mType = type;
    }
    ...
    private static class ItemViewHolder extends AbsViewBinder<PerformanceBean> {
        ...
        public void bind(final PerformanceBean bean) {
            ...
            if (values != null) {
                ...
                if (TYPE_FRAME == mType) {
                    String text = String.format("帧率平均值：%.2f", getAverage(values));
                    frame.setText(text);
            ...
            getView().setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    bundle.putInt(KEY_TYPE, getFragmentType());
        ...
        private int getFragmentType() {
            if (TYPE_FRAME == mType) {
                return TYPE_FRAME_ITEM;
            } else if (TYPE_CPU == mType) {
                return TYPE_CPU_ITEM;
            } else if (TYPE_MEMORY == mType) {
                return TYPE_MEMORY_ITEM;
            } else if (TYPE_BLOCK == mType) {
                return TYPE_BLOCK_ITEM;
            } else if (TYPE_LOAD_PAGE == mType) {
                return TYPE_LOAD_PAGE_ITEM;
            } else if (TYPE_UI_LAYER == mType) {
                return TYPE_UI_LAYER_ITEM;
            }
            return TYPE_CPU_ITEM;
        }
